...
public interface EmployeeAccessCardRepository extends JpaRepository<EmployeeAccessCard, Integer> {
    ...
    @Query("UPDATE EmployeeAccessCard eac SET eac.accessCard.active = false " +
            "WHERE eac.empId = :empId AND eac.returnDt IS NULL")
    ...
    // JPQL does not support direct INSERT INTO, we can use a constructor expression or use save method
    @Modifying
    @Transactional
    @Query("INSERT INTO EmployeeAccessCard (empId, accessCard, issuedDt) " +
            "VALUES (:empId, (SELECT ac FROM AccessCard ac WHERE ac.cardId = :cardId), CURRENT_TIMESTAMP)")
    ...
    @Query("UPDATE EmployeeAccessCard eac SET eac.returnDt = CURRENT_TIMESTAMP " +
            "WHERE eac.empId = :empId AND eac.accessCard.cardId = :cardId AND eac.returnDt IS NULL")
    ...
    @Query("SELECT eac FROM EmployeeAccessCard eac WHERE eac.empId = :empId AND eac.returnDt IS NULL")
